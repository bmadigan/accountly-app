        ...
        <div class="rounded-md bg-white shadow-xs">
            <div class="py-1">
                @foreach($teams as $team)
                    ...
                    @else
                        <a wire:click="switchTeam('{{ $team->id }}')"
                           class="block px-4 py-2 text-sm leading-5 cursor-pointer text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900">{{ $team->name }}</a>
                    @endif
                @endforeach
            </div>
